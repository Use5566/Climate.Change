...
紀錄分頁分別為 A學習歷程、B學習歷程、C學習歷程，均使用下述 11 欄。A、B 會保存完整對話，每次學生問題與 AI 回答分別保存版本；AI 失敗保留問題供重試。前台不能覆寫歷史對話或指定另一組 prompt。沒有金鑰不會產生假 AI 回答。本機 work/preview_ab.py 僅使用隔離的模擬 AI 與假名冊，未納入部署。
...
沿用現有服務帳戶憑證，該帳戶須具有指定試算表的「編輯者」權限。登入模組仍使用唯讀 scope；學習紀錄模組單獨使用 spreadsheets 讀寫 scope。此授權的範圍是整份試算表，程式僅對「A學習歷程」、「B學習歷程」、「C學習歷程」分頁寫入，不對 gid 0 寫入。

第一次開啟 C 頁面時會建立紀錄分頁與欄名。若已有同名分頁但欄名不符，停止寫入而不覆蓋（允許欄名首尾空白）。每次成功儲存新增一列，固定 A:K 共 11 欄：紀錄時間、班級、座號、密碼、介面、成績統計、立場、提交狀態、劃記原文、學生提問及AI回答內容、推論。時間使用臺灣時區；座號保留前導零；提交狀態為「草稿」或「已提交」。密碼、成績統計與 C 不使用的 AI 對話欄留白，未評分不填 0。

系統恢復資料（文章版本、劃記位置、草稿版本等）保存於 K 欄推論儲存格的備註，不增加表格欄位；與該列內容透過同一次 appendCells 請求寫入。所有顯示值使用明確的 stringValue，學生文字不會被當成公式執行。請勿修改／刪除原始紀錄及備註；可另製副本分析。CSV 下載可讀取 11 欄顯示內容，但不包含恢復備註，不能作為完整系統備份。既有舊版 14 欄不會自動轉換。

環境變數 LEARNING_STORAGE=google_sheets（預設），不需 LEARNING_DB_PATH。原 SQLite 紀錄不會自動搬移。現階段採單一 worker、單一服務實例；每次成功儲存留下版本，不代表每個點擊或按鍵都被捕捉。尚需線上權限驗證與班級同時操作的配額／負載測試。
