...
維持單一服務實例、單一 worker。後續教師下載功能尚待製作；備份資料庫即可保留目前的原始紀錄。C 文章請修改 prompts/interface_c.txt（UTF-8、空白行分段、**文字** 粗體），重新部署後載入；不必修改 Python。C 只有一篇固定文章，所有學生一律讀取這份 TXT，不從個別學生的紀錄備份選擇文章。既有學習紀錄不會自動刪除。
