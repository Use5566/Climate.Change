...
目前預設 Google Sheets 儲存使用 10 欄：A 紀錄時間、B 班級、C 座號、D 介面、E 立場、F 提交狀態、G 劃記原文、H 學生提問及AI回答內容、I 推論、J 對話總 tokens。登入用 gid 0 名冊不變，仍須保留登入密碼。下文提及 11 欄及 K 欄備註為舊版格式；新版會於各介面首次載入時原子轉換舊學習歷程，備註改存 I 欄並清空原 K 欄，不需教師手動刪欄。
...
維持單一服務實例、單一 worker。後續教師下載功能尚待製作；備份資料庫即可保留目前的原始紀錄。C 文章請修改 prompts/interface_c.txt（UTF-8、空白行分段、**文字** 粗體），重新部署後載入並自動產生內容版本；不必修改 Python。既有學生從原文章備份繼續，新學生使用新版。更換文章不會清除已提交紀錄，若要重做活動須先備份並另行規劃。
