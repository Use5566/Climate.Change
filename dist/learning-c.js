...
function save() {
  ...
  const operation = queue.then(async () => {
    ...
    const result = await api('/api/c/draft', payload());
...
function show(stage, focus = true) {
  ...
  if (stage === 'reading') renderArticle();
...
function updateSelectionButton() {
  ...
  button.textContent = !selected.length ? '先選取文章文字' : isCovered(work.highlights,selected) ? '取消這段劃記' : '劃記選取文字';
}
...
$('#inference-form').addEventListener('submit',async e => {
  ...
  try {
    await save();
    const result=await api('/api/c/submit',payload());
...
async function load() {
  try {
    ...
    $('#main').hidden=false; show(work.submitted_at ? 'summary' : work.stage,false);
  } catch(e) {error(e.message);}
